// Blaze Spear — tapered gold/red cursor blaze
// Adapted from ghostty-shader-playground WIP.glsl (MIT)
...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
    ...
    // Bar ↔ hollow-block on blur keeps the same origin; that is not a move.
    if (length(currentCursor.xy - previousCursor.xy) < currentCursor.w * 0.12) {
        return;
    }
